...
export class PaginaInicioComponent implements OnInit {
  ...
  firequery(start, end) {
    return this.afs.collection('actividades', ref => ref.limit(100).orderBy('userNombre').startAt(start).endAt(end)).valueChanges();
  }

  firequeryTitulo(start, end) {
    return this.afs.collection('actividades', ref => ref.limit(100).orderBy('actividad').startAt(start).endAt(end)).valueChanges();
  }
